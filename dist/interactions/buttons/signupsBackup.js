...
exports.default = new interactions_1.Button('signups-backup').onExecute((i, cache) => __awaiter(void 0, void 0, void 0, function* () {
    if (!cache)
    ...
    if (signup.isLocked)
        return i.reply({ content: 'This signup is locked.', ephemeral: true });
    const hasPlayer = signup.players.includes(i.user.id);
    const hasBackup = signup.players.includes(i.user.id);
    if (hasPlayer || hasBackup)
        return i.reply({ content: 'You cannot a join a game you already are in.', ephemeral: true });
    ...
    const { embed, row } = yield (0, signups_1.createSignupPost)(updatedSignup, i.guild);
    yield i.message.edit({ embeds: [embed], components: [row] });
    yield i.reply({ content: 'Successfully signed up', ephemeral: true });
    yield i.deleteReply();
}));
//# sourceMappingURL=signupsBackup.js.map